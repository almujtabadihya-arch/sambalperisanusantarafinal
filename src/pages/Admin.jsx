      ...
      <div style={{ display: 'flex', maxWidth: '1400px', margin: '0 auto', padding: '20px', gap: '20px' }}>
        ...
        <div style={{ flex: 1 }}>
          {activeTab === 'orders' ? (
            <div style={{ display: 'grid', gap: '15px' }}>
              {orders.length === 0 ? (
                ...
                orders.map((order) => {
                  ...
                  return (
                    <div key={order.id || order._id || Math.random()} style={{ background: 'white', padding: '20px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.05)', border: '1px solid #eee' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', borderBottom: '1px solid #f0f0f0', paddingBottom: '10px' }}>
                        <div>
                          <div style={{ fontWeight: '900', color: '#D32F2F' }}>{order.orderId || 'ORD-UNKNOWN'}</div>
                          <div style={{ fontSize: '0.8rem', color: '#888' }}>{order.date ? new Date(order.date).toLocaleString() : '-'}</div>
                        </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                        <div>
                          ...
                          <p style={{ fontWeight: 'bold' }}>{order.customer?.name || 'Anonim'}</p>
                        ...
                        <div>
                          ...
                          {Array.isArray(order.items) && order.items.map((item, i) => (
                            <div key={i} style={{ fontSize: '0.9rem' }}>{item?.name || 'Item'} x {item?.quantity || 1}</div>
                          ))}
                          <p style={{ fontWeight: '900', marginTop: '10px', fontSize: '1.1rem' }}>Total: Rp {order.totalAmount?.toLocaleString() || 0}</p>
                        </div>
                      </div>

                      <div style={{ marginTop: '20px', display: 'flex', gap: '10px' }}>
                        <button onClick={() => updateStatus(order.id, 'Diproses')} style={{ flex: 1, padding: '12px', background: '#E3F2FD', color: '#1565C0', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold' }}>⚡ PROSES</button>
                        <button onClick={() => updateStatus(order.id, 'Dikirim')} style={{ flex: 1, padding: '12px', background: '#F3E5F5', color: '#7B1FA2', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold' }}>🚚 KIRIM</button>
                        <button onClick={() => updateStatus(order.id, 'Selesai')} style={{ flex: 1, padding: '12px', background: '#E8F5E9', color: '#2E7D32', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold' }}>✅ SELESAI</button>
                      </div>
                    </div>
                  );
                })
            ...
            <div style={{ background: 'white', padding: '30px', borderRadius: '20px', minHeight: '400px' }}>
               <h2 style={{ marginBottom: '20px', fontWeight: '900' }}>💬 Chat Pelanggan</h2>
                 ...
                 Object.keys(chats).map(userId => {
                   ...
                   return (
                     <div key={userId} style={{ borderBottom: '1px solid #eee', padding: '15px 0' }}>
                       <p style={{ fontWeight: 'bold', color: '#D32F2F' }}>👤 {userId}</p>
                       <p style={{ fontSize: '0.95rem', color: '#444', marginTop: '5px' }}>{lastMsg?.text || 'Pesan gambar/file'}</p>
                       <p style={{ fontSize: '0.7rem', color: '#aaa' }}>{lastMsg?.timestamp ? new Date(lastMsg.timestamp).toLocaleString() : '-'}</p>
                     </div>
                   );
                 })
               )}
